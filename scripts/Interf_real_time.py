...
threshold_slider = Slider(threshold, "threshold vol", 0.0, 500.0, valinit=80.0)
clusters = Slider(num_clusters, "num clusters", 0.0, 40.0, valinit=3.0, color="cyan")
freq = Slider(axdownsample, "downsample", 0.001, 1.0, valinit=0.5)
amplitude = Slider(axradiusoutliers, "radius outliers", 0, 5.0, valinit=1.0)
neib = Slider(axnboutliers, "neib outliers", 0.0, 100.0, valinit=20.0)
zmax = Slider(z_max, "Z Max", 0.0, 1.0, valinit=0.154)
zmin = Slider(z_min, "Z Min", -1.0, 1.0, valinit=-0.157)
ymax = Slider(y_max, "Y Max", -4.0, 5.0, valinit=1.17)
ymin = Slider(y_min, "Y Min", -5.0, 4.0, valinit=-0.11)
xmax = Slider(x_max, "X Max", -4.0, 5.0, valinit=1.625)
xmin = Slider(x_min, "X Min", -5.0, 4.0, valinit=0.192)
...
def _pub_mark(event):
    ...
    print("Button pressed, starting to grasp objects")
    ...
    if resp.fail_id != -1:
        ...
        #### Text Output
        print("Failure at object with id", resp.fail_id)
    else:
        print("Grasp ok")
        #### Text Output
        plt.text(
...
def _extract_soil(event):
    global pcd_4_pub, pcl_pub
    plane_model, inliers = pcd_4_pub.segment_plane(
...
def update(val):
    global val_downs, pub, pcl_pub, pcd_4_pub, clust_default, tfBuffer
    global nube_local_recibida
    ...
    pcd = pc2.read_points(
        nube_local_recibida, skip_nans=True, field_names=("x", "y", "z")
    )
...
def talker():
    ...
    bcut_5 = Button(
        pub_arm_pose, "Pub Pose Arm Inspect", color="white", hovercolor="red"
    )
    bcut_5.on_clicked(_pub_arm_pose)

    bcut_6 = Button(
        get_optimal_order,
        "Order Objects for Grasping",
        color="white",
        hovercolor="yellow",
    )
